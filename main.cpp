    ...
    if (game->winner == 1) {
        ...
        for (move_column = 0; move_column < 7; move_column++) {
            ...
            if (player == 2) {
                ...
                if (next_level > max_score) {
                    column = move_column;
                    max_score = next_level;
                }
            }
        }
        if (player == 2) {return min_score;}
...
int bestMove(int depth, int max_depth, int player, struct Game * game){
    ...
    for (move_column = 0; move_column < 7; move_column++) {
        ...
        enterMove(game_clone, player);
        next_level = miniMax(depth - 1, max_depth, 2, game_clone);
        if (next_level > maxScore && max_depth == depth) {
            maxScore = next_level;
            column = move_column;
        }
    }
...
//запрашиваем ход пользователя или вызываем минимакс для хода бота
